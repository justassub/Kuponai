...
import lt.it.etalonas.Services.CouponServices;
...
public class CouponController {
    ...
    public  void editCoupon(@Valid @PathVariable Long id, @RequestBody CreateCouponCommand ccc){
        couponServices.editCoupon(id,ccc);
    }

    @RequestMapping(path = "/create",method = RequestMethod.POST)
    @ApiOperation(value = "creates one coupon",notes = "creates a coupon by tatking data")
    public  void createCoupon(@Valid @RequestBody CreateCouponCommand ccc){
    ...
    @RequestMapping(path = "/worth/{id}",method = RequestMethod.GET)
    public double getProductWorth(@PathVariable long id){return couponServices.getProductWorth(id);}
